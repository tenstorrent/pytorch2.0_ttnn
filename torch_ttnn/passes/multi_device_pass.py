...
class MultiDevicePass(PassBase):
    ...
    def call(self, gm: torch.fx.GraphModule):
        ...
        for node in node_list:
            ...
            if node.op == "placeholder":
                try:
                    if (node.meta["primal_tag"] != PrimalTag.PARAMETER) and (
    ...
    def rewrite_const_args(self, gm: torch.fx.GraphModule, node):
        ...
        if node.target == torch.ops.aten.view.default:
            output_dim = node.args[1]
            output_dim[node.meta.shard_dim] = get_sharded_size(output_dim[node.meta.shard_dim])
            node.args[1] = output_dim
