...
def evict_tensors_from_device(gm: torch.fx.GraphModule,
                              mm: MemoryManager,
                              guilty_op: str,    
                              tensors_to_evict: list) -> torch.fx.GraphModule:
    ...
    for node in topo_sorted_nodelist:
        ...
        if tid in tensors_to_evict:
            assert node.name != guilty_op, "Op which overflows the SRAM can't have its output tensor evicted from device"
            evict_node = node
