...
at::Tensor custom_empty_memory_format(
    ...
    auto tensor = ttnn::empty(
        ttnn::Shape(small_vector),
...
at::Tensor custom__copy_from(const at::Tensor& self, const at::Tensor& dst, bool non_blocking) {
    ...
    if (self.is_cpu() && dst.device().type() == c10::DeviceType::PrivateUse1) {
        ...
        if (dtype == ttnn::DataType::UINT32) {
            ...
            std::transform(
                src_cpu_vector.begin(),
                src_cpu_vector.end(),
                std::back_inserter(src_cpu_vector_int),
            ...
            if (logical_rank == 1) {
                ...
                if (dtype == ttnn::DataType::UINT32) {
                    ...
                    std::transform(
                        src_reshaped_vector.begin(),
                        src_reshaped_vector.end(),
                        std::back_inserter(src_vector_int),
                        [](const uint32_t value) { return static_cast<int>(value); });
                ...
                std::vector<int> src_vector_int;
                std::transform(
                    src_vector.begin(), src_vector.end(), std::back_inserter(src_vector_int), [](const uint32_t value) {
...
at::Tensor custom_empty_strided(
    ...
    auto ttnn_tensor_ret =
        at::detail::make_tensor<at::TtnnTensorImpl>(private_use_ks, dtype, device, ttnn_tensor, storage_impl);
