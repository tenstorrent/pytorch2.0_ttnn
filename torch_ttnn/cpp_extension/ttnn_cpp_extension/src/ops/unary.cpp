...
#include <ttnn/tensor/types.hpp>   // for DataType (replacement for DType)
...
at::Tensor ttnn_gelu(const at::Tensor& self, c10::string_view approximate) {
    ...
    TORCH_CHECK(self.device().type() == c10::DeviceType::PrivateUse1,
                "ttnn_gelu: only PrivateUse1 device is supported");

    TORCH_CHECK(approximate == "none",
                "ttnn_gelu: only approximate='none' is supported");
