...
def node_to_python_code(node):
    ...
    # simplify or put this in a new function
    opname = str(node.target) if str(node.target).startswith("aten.") else node.target.__name__
    if not opname.startswith("aten.") and not opname.startswith("ttnn.") and not isinstance(node.target, types.BuiltinFunctionType):
    ...
    replace_map = {"ttnn_Specified_Device": "device", "ttnn_TILE_LAYOUT": "ttnn.TILE_LAYOUT", "ttnn_ROW_MAJOR_LAYOUT": "ttnn.ROW_MAJOR_LAYOUT", "ttnn_L1_MEMORY_CONFIG": "ttnn.L1_MEMORY_CONFIG", "ttnn_DRAM_MEMORY_CONFIG": "ttnn.DRAM_MEMORY_CONFIG", "ttnn_uint32": "ttnn.uint32", "ttnn_bfloat16": "ttnn.bfloat16"}
    ...
    for node in ttnn_graph.nodes:
        ...
        if "seq_nr" in node.meta:
            ...
            if node == aten_to_ttnn_map[aten_node][-1]:
                # this will be converted to compare(node1, node2) later
...
def generate_op_accuracy_tests(model_name, aten_fx_graphs, ttnn_fx_graphs, all_inputs, *, verbose = False):
    ...
    for aten_graph, ttnn_graph in zip(aten_fx_graphs, ttnn_fx_graphs):
        ...
        # interleave aten with ttnn code and insert compare code at the end of each section
        ...
        for node in ttnn_all_nodes:
            if isinstance(node, tuple):
                graph_code.append(f"  compare({node[0]}, {node[1]})")
            else:
                graph_code.append(f"  {node_to_python_code(node)}")
        graph_code.append("  ttnn.close_device(device)")

        compare_graph_codes.append("\n".join(graph_code))
...
if __name__ == "__main__":
    file = open("{model_name}_inputs.pickle", "rb")
    inputs = pickle.load(file)
    forward(*inputs)
"""

    # test functions
    pcc_funcs = [inspect.getsource(comp_pcc), inspect.getsource(construct_pcc_assert_message), inspect.getsource(assert_with_pcc)]

    full_code = import_code + pcc_funcs + wrapper_code + [compare_code] + compare_graph_codes + [main_code]
    full_text = "\n".join(full_code)
    
    with open(f"{model_name}_compare_code.py", "w") as text_file:
        print(full_text, file=text_file)
    
    if all_inputs is not None:
        with open(f"{model_name}_inputs.pickle", "wb") as f:
