...
def plot_bar_chart(src_file, dest_file):
    ...
    for i, tensors in enumerate(data_points_mb):
        ...
        for j, (tensor_id, size) in enumerate(tensors):
            plt.bar(i * (bar_width + bar_spacing), size, bottom=bottom, color=colors(i), edgecolor='black', width=bar_width)
    ...
    plt.legend(handles=[plt.Line2D([0], [0], color='red', linestyle='--', linewidth=2)], 
