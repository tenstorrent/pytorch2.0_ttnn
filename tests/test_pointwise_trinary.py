    ...
    def _test_addcop_impl(self, source_op, target_op, with_value):
        class AddcmulModule(torch.nn.Module):
            ...
            def input_types(self):
                return [((4, 4), torch.float16), ((4, 4), torch.float16), ((4, 4), torch.float16)]
        ...
        self.assertTrue(nodes[12].target == target_op)
        ...
        self.assertTrue(torch.allclose(result_before.to(torch.bfloat16), result_after.to(torch.bfloat16)))

    def test_addcmul(self):
        self._test_addcop_impl(source_op=torch.addcmul, target_op=ttnn.addcmul, with_value=False)

    def test_addcmul_with_value(self):
        self._test_addcop_impl(source_op=torch.addcmul, target_op=ttnn.addcmul, with_value=True)

    def test_addcdiv(self):
        self._test_addcop_impl(source_op=torch.addcdiv, target_op=ttnn.addcdiv, with_value=False)

    def test_addcdiv_with_value(self):
        self._test_addcop_impl(source_op=torch.addcdiv, target_op=ttnn.addcdiv, with_value=True)
