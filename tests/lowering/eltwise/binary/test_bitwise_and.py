...
def test_bitwise_and_scalar(device, input_shape):
    m = BitwiseAndModule()
    ...
    assert [node.target for node in nodes].count(ttnn.bitwise_and) == 1
...
def test_bitwise_and_tensor(device, input_shapes):
    m = BitwiseAndModule()
