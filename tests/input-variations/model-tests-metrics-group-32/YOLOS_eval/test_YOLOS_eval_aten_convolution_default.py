...
def teardown_module(module):
    print(metrics)
    save_pickle(metrics, "metrics-input-variations/model-tests-metrics-group-32/YOLOS eval", "aten.convolution.default")


@pytest.mark.parametrize("input_strings", [['Tensor<[1, 3, 512, 672]> input = ?', 'Tensor<[192, 3, 16, 16]> weight = ?', 'Optional[Tensor]<[192]> bias = ?', 'List[int] stride = [16, 16]', 'List[int] padding = [0, 0]', 'List[int] dilation = [1, 1]', 'bool transposed = False', 'List[int] output_padding = [0, 0]', 'int groups = 1']])
