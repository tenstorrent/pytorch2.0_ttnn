...
def teardown_module(module):
    print(metrics)
    save_pickle(metrics, "metrics-input-variations/model-tests-metrics-group-11/SegFormer eval", "aten._native_batch_norm_legit_no_training.default")


@pytest.mark.parametrize("input_strings", [['Tensor<[1, 256, 128, 128]> input = ?', 'Optional[Tensor]<[256]> weight = ?', 'Optional[Tensor]<[256]> bias = ?', 'Tensor<[256]> running_mean = ?', 'Tensor<[256]> running_var = ?', 'float momentum = 0.1', 'float eps = 1e-05']])
