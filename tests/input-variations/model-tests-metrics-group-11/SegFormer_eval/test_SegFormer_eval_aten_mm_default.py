...
def teardown_module(module):
    print(metrics)
    save_pickle(metrics, "metrics-input-variations/model-tests-metrics-group-11/SegFormer eval", "aten.mm.default")


@pytest.mark.parametrize("input_strings", [['Tensor<[16384, 32]> self = ?', 'Tensor<[32, 256]> mat2 = ?'], ['Tensor<[4096, 64]> self = ?', 'Tensor<[64, 256]> mat2 = ?'], ['Tensor<[1024, 160]> self = ?', 'Tensor<[160, 256]> mat2 = ?'], ['Tensor<[256, 256]> self = ?', 'Tensor<[256, 256]> mat2 = ?']])
