...
def teardown_module(module):
    print(metrics)
    save_pickle(metrics, "metrics-input-variations/model-tests-metrics-group-9/OpenPose V2 eval", "aten.relu.default")


@pytest.mark.parametrize("input_strings", [['Tensor<[1, 32, 112, 112]> self = ?'], ['Tensor<[1, 64, 112, 112]> self = ?'], ['Tensor<[1, 64, 56, 56]> self = ?'], ['Tensor<[1, 128, 56, 56]> self = ?'], ['Tensor<[1, 128, 28, 28]> self = ?'], ['Tensor<[1, 256, 28, 28]> self = ?'], ['Tensor<[1, 512, 28, 28]> self = ?']])
