...
def teardown_module(module):
    print(metrics)
    save_pickle(metrics, "metrics-input-variations/model-tests-metrics-group-3/Autoencoder (linear) eval", "aten.relu.default")


@pytest.mark.parametrize("input_strings", [['Tensor<[1, 128]> self = ?'], ['Tensor<[1, 64]> self = ?'], ['Tensor<[1, 12]> self = ?']])
