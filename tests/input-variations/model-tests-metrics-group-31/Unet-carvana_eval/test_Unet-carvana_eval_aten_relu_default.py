...
def teardown_module(module):
    print(metrics)
    save_pickle(metrics, "metrics-input-variations/model-tests-metrics-group-31/Unet-carvana eval", "aten.relu.default")


@pytest.mark.parametrize("input_strings", [['Tensor<[1, 64, 224, 224]> self = ?'], ['Tensor<[1, 128, 112, 112]> self = ?'], ['Tensor<[1, 256, 56, 56]> self = ?'], ['Tensor<[1, 512, 28, 28]> self = ?'], ['Tensor<[1, 1024, 14, 14]> self = ?']])
