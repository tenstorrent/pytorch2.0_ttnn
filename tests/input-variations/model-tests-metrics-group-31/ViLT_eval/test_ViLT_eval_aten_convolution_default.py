...
def teardown_module(module):
    print(metrics)
    save_pickle(metrics, "metrics-input-variations/model-tests-metrics-group-31/ViLT eval", "aten.convolution.default")


@pytest.mark.parametrize("input_strings", [['Tensor<[1, 3, 384, 512]> input = ?', 'Tensor<[768, 3, 32, 32]> weight = ?', 'Optional[Tensor]<[768]> bias = ?', 'List[int] stride = [32, 32]', 'List[int] padding = [0, 0]', 'List[int] dilation = [1, 1]', 'bool transposed = False', 'List[int] output_padding = [0, 0]', 'int groups = 1']])
