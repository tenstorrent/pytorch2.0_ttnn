...
def teardown_module(module):
    print(metrics)
    save_pickle(metrics, "metrics-input-variations/model-tests-metrics-group-31/Unet-brain eval", "aten.relu.default")


@pytest.mark.parametrize("input_strings", [['Tensor<[1, 32, 256, 256]> self = ?'], ['Tensor<[1, 64, 128, 128]> self = ?'], ['Tensor<[1, 128, 64, 64]> self = ?'], ['Tensor<[1, 256, 32, 32]> self = ?'], ['Tensor<[1, 512, 16, 16]> self = ?']])
