...
def teardown_module(module):
    print(metrics)
    save_pickle(metrics, "metrics-input-variations/model-tests-metrics-group-24/vit_h_14 eval", "aten.convolution.default")


@pytest.mark.parametrize("input_strings", [['Tensor<[1, 3, 518, 518]> input = ?', 'Tensor<[1280, 3, 14, 14]> weight = ?', 'Optional[Tensor]<[1280]> bias = ?', 'List[int] stride = [14, 14]', 'List[int] padding = [0, 0]', 'List[int] dilation = [1, 1]', 'bool transposed = False', 'List[int] output_padding = [0, 0]', 'int groups = 1']])
