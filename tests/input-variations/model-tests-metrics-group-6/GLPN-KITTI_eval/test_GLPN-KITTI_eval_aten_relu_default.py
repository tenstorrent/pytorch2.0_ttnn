...
def teardown_module(module):
    print(metrics)
    save_pickle(metrics, "metrics-input-variations/model-tests-metrics-group-6/GLPN-KITTI eval", "aten.relu.default")


@pytest.mark.parametrize("input_strings", [['Tensor<[1, 64, 30, 40]> self = ?'], ['Tensor<[1, 32, 30, 40]> self = ?'], ['Tensor<[1, 64, 60, 80]> self = ?'], ['Tensor<[1, 32, 60, 80]> self = ?'], ['Tensor<[1, 64, 120, 160]> self = ?'], ['Tensor<[1, 32, 120, 160]> self = ?'], ['Tensor<[1, 64, 480, 640]> self = ?']])
