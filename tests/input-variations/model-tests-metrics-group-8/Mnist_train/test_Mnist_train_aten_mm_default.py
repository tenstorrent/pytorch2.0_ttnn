...
def teardown_module(module):
    print(metrics)
    save_pickle(metrics, "metrics-input-variations/model-tests-metrics-group-8/Mnist train", "aten.mm.default")


@pytest.mark.parametrize("input_strings", [['Tensor<[1, 10]> self = ?', 'Tensor<[10, 128]> mat2 = ?'], ['Tensor<[10, 1]> self = ?', 'Tensor<[1, 128]> mat2 = ?'], ['Tensor<[1, 128]> self = ?', 'Tensor<[128, 9216]> mat2 = ?'], ['Tensor<[128, 1]> self = ?', 'Tensor<[1, 9216]> mat2 = ?']])
