...
def teardown_module(module):
    print(metrics)
    save_pickle(metrics, "metrics-input-variations/model-tests-metrics-group-8/Mnist train", "aten.view.default")


@pytest.mark.parametrize("input_strings", [['Tensor<[1, 64, 12, 12]> self = ?', 'List[int] size = [1, 9216]'], ['Tensor<[1, 10]> self = ?', 'List[int] size = [10]'], ['Tensor<[1, 128]> self = ?', 'List[int] size = [128]'], ['Tensor<[1, 9216]> self = ?', 'List[int] size = [1, 64, 12, 12]']])
