...
def teardown_module(module):
    print(metrics)
    save_pickle(metrics, "metrics-input-variations/model-tests-metrics-group-29/swin_v2_b eval", "aten.convolution.default")


@pytest.mark.parametrize("input_strings", [['Tensor<[1, 3, 256, 256]> input = ?', 'Tensor<[128, 3, 4, 4]> weight = ?', 'Optional[Tensor]<[128]> bias = ?', 'List[int] stride = [4, 4]', 'List[int] padding = [0, 0]', 'List[int] dilation = [1, 1]', 'bool transposed = False', 'List[int] output_padding = [0, 0]', 'int groups = 1']])
