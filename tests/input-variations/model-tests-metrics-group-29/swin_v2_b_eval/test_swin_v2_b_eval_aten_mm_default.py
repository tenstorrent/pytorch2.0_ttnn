...
def teardown_module(module):
    print(metrics)
    save_pickle(metrics, "metrics-input-variations/model-tests-metrics-group-29/swin_v2_b eval", "aten.mm.default")


@pytest.mark.parametrize("input_strings", [['Tensor<[225, 512]> self = ?', 'Tensor<[512, 4]> mat2 = ?'], ['Tensor<[1024, 512]> self = ?', 'Tensor<[512, 256]> mat2 = ?'], ['Tensor<[225, 512]> self = ?', 'Tensor<[512, 8]> mat2 = ?'], ['Tensor<[256, 1024]> self = ?', 'Tensor<[1024, 512]> mat2 = ?'], ['Tensor<[225, 512]> self = ?', 'Tensor<[512, 16]> mat2 = ?'], ['Tensor<[64, 2048]> self = ?', 'Tensor<[2048, 1024]> mat2 = ?'], ['Tensor<[225, 512]> self = ?', 'Tensor<[512, 32]> mat2 = ?']])
