...
def teardown_module(module):
    print(metrics)
    save_pickle(metrics, "metrics-input-variations/model-tests-metrics-group-16/dla34.in1k eval", "aten.relu.default")


@pytest.mark.parametrize("input_strings", [['Tensor<[1, 16, 224, 224]> self = ?'], ['Tensor<[1, 32, 112, 112]> self = ?'], ['Tensor<[1, 64, 56, 56]> self = ?'], ['Tensor<[1, 128, 28, 28]> self = ?'], ['Tensor<[1, 256, 14, 14]> self = ?'], ['Tensor<[1, 512, 7, 7]> self = ?']])
