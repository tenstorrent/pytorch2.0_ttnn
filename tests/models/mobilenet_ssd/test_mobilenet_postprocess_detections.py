...
class ThisTester(ModelTester):
    def _load_model(self):
        ...
        return self.full_model.postprocess_detections # .to(torch.bfloat16)
    ...
    def _load_inputs(self):
        ...
        inputs = batch_tensor
        features = self.full_model.backbone(inputs.tensors)
        if isinstance(features, torch.Tensor):
            features = OrderedDict([("0", features)])
        features = list(features.values())
        head_outputs = self.full_model.head(features)
        anchors = self.full_model.anchor_generator(inputs, features)
        return (head_outputs, anchors, inputs.image_sizes)
...
def test_mobilenet_ssd(record_property, mode):
    model_name = "MobileNetSSD_postprocess_detections"
