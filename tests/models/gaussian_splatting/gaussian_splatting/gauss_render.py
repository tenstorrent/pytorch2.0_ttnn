# SPDX-FileCopyrightText: https://github.com/hbb1/torch-splatting/blob/6c9792/gaussian_splatting/gauss_render.py © 2025 Binbin Huang
...
def build_scaling_rotation(s, r):
    L = torch.zeros((s.shape[0], 3, 3), device="cpu")
    R = build_rotation(r)
...
def strip_lowerdiag(L):
    uncertainty = torch.zeros((L.shape[0], 6), device="cpu")
    uncertainty[:, 0] = L[:, 0, 0]
    uncertainty[:, 1] = L[:, 0, 1]
    uncertainty[:, 2] = L[:, 0, 2]
    uncertainty[:, 3] = L[:, 1, 1]
...
class GaussRenderer(nn.Module):
    ...
    def render(self, camera, means2D, cov2d, color, opacity, depths):
        ...
        self.render_color = torch.ones(*self.pix_coord.shape[:2], 3)
        self.render_depth = torch.zeros(*self.pix_coord.shape[:2], 1)
        self.render_alpha = torch.zeros(*self.pix_coord.shape[:2], 1)
        ...
        for h in range(0, camera.image_height, TILE_SIZE):
            for w in range(0, camera.image_width, TILE_SIZE):
                ...
                sorted_conic = sorted_cov2d.to(torch.float).inverse().to(torch.float16)  # inverse of variance
    ...
    def forward(self, camera, pc, **kwargs):
        ...
        with prof("projection"):
            mean_ndc, mean_view, in_mask = projection_ndc(
                means3D, viewmatrix=camera.world_view_transform, projmatrix=camera.projection_matrix
            )
