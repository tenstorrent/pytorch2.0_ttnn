...
def teardown_module(module):
    print(metrics)
    save_pickle(metrics, "metrics-autogen-op/BERT", "aten.mul.Scalar")


@pytest.mark.parametrize(
    "input_strings", [
        ["Tensor<[1, 16, 256, 256]> self = ?", "float other = 2.0"],
        ["Tensor<[1, 768]> self = ?", "float other = 0.5"],
        ["Tensor<[1, 512, 768]> self = ?", "float other = 1.5"],
        ["Tensor<[12, 512, 64]> self = ?", "float other = 3.0"],
    ]
)
