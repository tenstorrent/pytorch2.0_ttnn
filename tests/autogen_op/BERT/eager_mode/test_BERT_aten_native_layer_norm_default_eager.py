...
def test_aten(device, input_strings, input_var_only_native, input_var_check_accu, input_var_check_ttnn):
    ...
    if metric["run"] == True:
        try:
            # Check inference result - layer norm returns a tuple (output, mean, rstd)
            if isinstance(result_before, tuple) and isinstance(result_after, tuple):
                # Compare only the main output (first element of the tuple)
                metric["accuracy"] = calculate_accuracy(result_before[0], result_after[0])
            else:
