...
@pytest.mark.parametrize(
    "input_strings", [
        ...
        ["Tensor<[1, 768]> self = ?", "number other = 1.0"],
        ["Tensor<[1, 512, 768]> self = ?", "number other = 1.0"],
        ["Tensor<[12, 512, 64]> self = ?", "number other = 0.0"],
        ["Tensor<[1, 256, 1024]> self = ?", "number other = 2.0"],
        ["Tensor<[2, 1024, 512]> self = ?", "number other = -1.0"],
        ["Tensor<[8, 128, 3072]> self = ?", "number other = 0.5"],
    ]
)
def test_aten(device, input_strings, input_var_only_native, input_var_check_accu, input_var_check_ttnn):
    ...
    input_args, input_kwargs, status = render_metric_string_list_to_input_args_kwargs(
        "aten.rsub.Scalar", input_strings
    )
    if status == False:
